...
// });
  ...
  rbsm.find({rbsmid : gname_id}, {_id:0, rbsmplist:1}, function(err, pLists){
   ...
   console.log('gname =' +gname );
    res.render("htable/rb_each", {pList:pListi,  gname:gname, gameID:gameID,
               hlow:hlow,hcol:hcol,gordermulti:gordermulti, doneArr:doneArr});
}); // find
}); // router
...
router.post("/savescore/:id", function(req, res){
  ...
  function(pListi, playerArr, noplayer, callback){
    ...
    // console.log('pListi[1][2]='+pListi[1][2]);
    // console.log('pListi[1][3]='+pListi[1][3]);
    // console.log('noplayer='+noplayer);
...
// 5동 2-2 처리 2 #############################[33-2-11]#####
  ...
  //   for ( var i = 0 ; i < noplayer ; i++ ){
  //     for ( var j = 0 ; j < noplayer ; j++ ){
  //           if(pListi[i+1][j+2] && pListi[j+2][i+1]){
  //             dlist.push([i+1] + "-" + [j+2]);
  //           }
  //     }
  //   };
